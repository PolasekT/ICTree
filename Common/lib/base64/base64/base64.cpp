...
std::string base64_encode(std::vector<uint8_t> const &s, bool url)
{
    return encode(s, url);
}
...
std::string base64_encode_pem(std::vector<uint8_t> const &s)
{
    return encode_pem(s);
}
...
std::string base64_encode_mime(std::vector<uint8_t> const &s)
{
    return encode_mime(s);
}
