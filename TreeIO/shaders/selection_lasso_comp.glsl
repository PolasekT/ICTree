 ...
 * @date 4.21.2020
 * @version 1.0
 * @brief Compute shader for point lasso selection.
 */
...
/// Number of valid cursor positions in the uCursorPositions array.
uniform int uCursorPosCount;
/// List of cursor positions making up the polygonal lasso.
uniform vec2 uCursorPositions[15];
...
/// @brief Check if given point P is on the left side of the line from A to B.
bool testLineIntersect(vec2 P, vec2 A, vec2 B)
{
    // Quick termination conditions:
    if (P.y < A.y && P.y < B.y)
    { return false; }
    if (P.y > A.y && P.y > B.y)
    { return false; }
    ...
    const vec2 AB = B - A;
    const vec2 AP = P - A;
    const vec2 projP = A + vec2(AB.x * (AP.y/AB.y), AP.y);
...
/// @brief Is given point inside polygon defined by provided array of points?
bool testPolygonIntersect(vec2 P, vec2 points[15], int pointCount)
{
    // Count the left intersections with the polygon.
    uint count = 0;
    int iii = 0;
    do
    {
        const int next = (iii + 1) % pointCount;

        const bool res = testLineIntersect(P, points[iii], points[next]);
        if (res)
        { count++; }
...
void main()
{
    ...
    const bool selected = isInFront && testPolygonIntersect(ndcPos.xy, uCursorPositions, uCursorPosCount);
